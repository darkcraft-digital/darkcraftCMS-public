$(document).ready(function(e){
    $("body").prepend('<div id="cms_navigation"><div id="cms_noticebar"><img id="cms_close" src="/cms/core/style/icons/close.svg"/></div></div>');
    $("#cms_close").click(function(){
        jo_close_noticebar($("#cms_noticebar"),true);
    });
});
...
function jo_ajax(url,data,command,asyncr = true){
  $.ajax({
    ...
    async: asyncr,
    success: function(e) {
        if(cms_error_handler(e)==true){
            command(e,data,url);
        }
    },
    error: function(xhr, textStatus, errorThrown){
        cms_noticebar_func(textStatus);
        return false;
    }
  });
}
...
//loads language
jo_ajax('/cms/core/inc/lang.php','',jo_language,false);
...
function jo_close_noticebar(elem,all){
  $(elem).slideUp(700,function(){
    ...
    if($("#cms_noticebar div").length == 0 || all == true){
      $("#cms_noticebar").slideUp(100,function(){
        var save = $("#cms_noticebar img").detach();
        $("#cms_noticebar").empty().append(save);
      });
    }
  });
}
...
//return: -
function cms_noticebar_func(token,message){
    console.log(token);
    ...
    $("#cms_noticebar").slideDown(50,"swing");
    var new_elem = $("<div></div>").text(message_return);
    $("#cms_noticebar").append(new_elem);
...
function cms_error_handler(e){
    console.log(e);
    if (e.error.status !=true){
        cms_noticebar_func('standard',e.error.message);
        return false;
    }else{
        return true;
    }
...
function jo_actionwindow(cb_save,cb_exit){
  ...
  $('#cms_navigation').append('<div id="cms_window_wrapper"></div><div id="cms_popup"><a id="cms_closepopup"><img src="/cms/core/style/icons/closeblack.svg"/></a><h1>'+jo_lang.ED_PROP+'</h1><div></div></div>');       //fehler, weil cms_markup aufgerufen wird
  $("body").prepend('<div id="cms_functions"><button class="jo_panel_drag"></button><div id="cms_functions_submit"><div id="cms_functions_submit_save" title="'+jo_lang.FORM_SAVE+'">'+jo_lang.FORM_OK+'</div><div id="cms_functions_submit_cancel" title="'+jo_lang.FORM_DISM_DESC+'"><img src="/cms/core/style/icons/closeblack.svg" alt="'+jo_lang.FORM_DISM+'" /></div></div><div id="cms_buttons"></div></div>');
  $('#cms_functions_submit_save').click(cb_save);
  $('#cms_functions_submit_cancel').click(cb_exit);
  $("#cms_functions").draggable({
      cancel: false,
      handle: ".jo_panel_drag"
    });
  $("#cms_closepopup, #cms_window_wrapper").click(function(){
      cms_closewindow();
  });
}
...
function cms_closewindow(){
    $("#cms_popup, #cms_window_wrapper").css("display","none");
    $("#cms_popup div").html("");
}
...
function jo_window(opt, callback){
    ...
    $("#cms_popup, #cms_window_wrapper").css("display","block");

    //create form
    $("#cms_popup>div").append('<form class="cms_form_attr jo_form" method="post" action="" enctype="multipart/form-data"><input type=\'submit\' value=\''+jo_lang.FORM_OK+'\'/></form>')

    //attr-form submit
    $(".cms_form_attr").submit(function(e){
        e.preventDefault();
        var attr = {};

        $(".cms_form_attr input[type='radio']:checked, .cms_form_attr input:not([type='submit'],[type='radio'])").each(function (i){
