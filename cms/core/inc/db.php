...
function cms_db_link(){
    return new SQLite3(JO_ROOT.'/cms/core/database/database.db');
}

//checks login for users, checks if brute force
//input: mail, password
//return: true if successfully logged in
function cms_login_check($name,$password){
        $now = time();
...
function cms_task_check($path,$type){
    $handle = cms_db_link();

    if($type == 'loggedin'){
        $result = $handle->query("SELECT COUNT(id) AS total FROM tasks WHERE user_id='".$handle->escapeString($_SESSION['cms_id'])."' AND task='".$handle->escapeString($path)."'");    //escapen
        $output = $result->fetchArray();
        if($output['total']  !== 0){
            return true;
        }
...
//return: true if success
function cms_task_insert($path,$code){
    $handle = cms_db_link();
    $return = $handle->exec("INSERT INTO tasks(task,user_id,time,code) VALUES ('".$handle->escapeString($path)."','".$handle->escapeString($_SESSION['cms_id'])."',datetime('now'),'".$handle->escapeString($code)."')");   //escapen
    return $return;
}
...
function cms_blacklist_check($path){
    $handle = cms_db_link();
    $result = $handle->query("SELECT COUNT(ban) AS total FROM rights WHERE user_id='".$handle->escapeString($_SESSION['cms_id'])."' AND ban='".$handle->escapeString($path)."'");    //escapen
    $output = $result->fetchArray();
    if($output ['total']  > 0){
        return false;
    }else{
...
//return: true if success
function cms_history($path,$code){
    $handle = cms_db_link();
    $return = $handle->exec("INSERT INTO history(task,user_id,time,code) VALUES ('".$handle->escapeString($path)."','".$handle->escapeString($_SESSION['cms_id'])."',datetime('now'),'".$handle->escapeString($code)."')");
