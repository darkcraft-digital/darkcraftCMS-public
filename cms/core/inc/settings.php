...
function jo_get_settings(){
  $settings = file_get_contents(JO_ROOT."/cms/core/settings/settings.ini");
...
function jo_set_settings($settings){
  ...
  if(file_put_contents(JO_ROOT."/cms/core/settings/settings.ini",$string)== false){
    return false;
  }
  $GLOBALS['JO_S'] = $old;
...
function jo_language($lang,$path = "/cms/core/lang/"){
  $url = $path.$lang.".json";
  if(jo_repair_url($url,array("json"))==false){
    $url = "/cms/core/lang/en.json";
  }
...
function jo_language_available(){
  return glob(JO_ROOT."/cms/core/lang/*.json");
}
