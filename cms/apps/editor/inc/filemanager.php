...
try{
    ...
    function replacement($url){
        $includings = '
            <!-- CMS INCLUDINGS -->
            <script type="text/javascript" src="/cms/apps/editor/js/manager.js"></script>
            <!-- /CMS INCLUDINGS -->
            ';
        return $includings;
    }
    ...
    if(strpos($filecontent, 'data-jo="true"', 0)!== false OR strpos($filecontent, "data-jo='cms'", 0)!== false){
       ...
       //insert positions of editable contents
        ...
        if(cms_task_insert($url,$filecontent)==false){
        ...
        $replacement = '<?php if($_GET["cms"] != "'.$pin.'"){exit("Access denied!");} ?>';
