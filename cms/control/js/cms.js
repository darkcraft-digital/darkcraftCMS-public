...
function jo_frame_ready(){
    ...
    history.pushState({info: "cms"},"cms","?path=" + encodeURIComponent(url));

    var iframedoc = iframe.contentDocument || iframe.contentWindow.document;
    if(iframedoc.querySelectorAll("*[data-jo='true'], *[data-cms='cms']").length != 0){
        $("#edit").remove();
        $("#buttons").append('<div id="edit" onclick="jo_openfile(\'' + url + '\')"><img src="/cms/core/style/icons/edit.svg" title="'+jo_lang.CMS_EDIT+'"/></div>');
    }else{
        $("#edit").remove();
    }

    $("#areaselector").attr("href","/cms/apps/areaselector/index.php?file=" + encodeURIComponent(url));
    $("#code").attr("href","/cms/apps/codeeditor/index.php?file=" + encodeURIComponent(url));
    $("#history").attr("href","/cms/apps/history/index.php?file=" + encodeURIComponent(url));
...
$(document).ready(function(){
        document.getElementById("frame").contentWindow.addEventListener('error', function (error) {
          cms_noticebar_func('js_error','File: ' + error.error.fileName + ' Location: ' + error.error.lineNumber + '/' + error.error.columnNumber);
        });
        function cms_explorer(e){
            var type = $(e).attr("id");
        ...
        function jo_explorer_toggle(e){
          ...
          $(e).append("<img src='/cms/core/style/icons/close.svg' class='explorer_close' title='"+jo_lang.FORM_DISM+"' />");
          if($("#navigation").width() < 50){
        ...
        $(".jo_folder").each(function(){
            if($(this).parent().find(".cms_file").length == 0){
              $(this).addClass("jo_folder_inactive");
              $(this).css({"pointer-events":"none"});
            }
        });

        $(".cms_file a").click(function(){
...
function jo_openfile(url){
  ...
  jo_ajax('/cms/apps/editor/inc/filemanager.php',content_complete,function(e,data,url){
      window.location.assign(decodeURIComponent(e.url)+ "_cms_temp.php?cms=" +  e.pin);
  });
}
